...
NOTE: The π character represents the number PI and is entered by pressing XXXXXXXXXXXX on the keyboard. If you are not using an official Commander X16 keyboard,
then you would need to do XXXXXXXXXXXXXXXXX instead.  ***** NOT SURE IF THIS WORTHY TO BE IN A VISUAL /note BLUE BOX OR NOT?\\
...
Once dimensioned, arrays can be used just like other variables, except
...
Executing a {\ttfamily DIM} statement on the same array more than once will cause an
...
character at a time.  It is comparable ***SIMILAR?*** to the {\ttfamily GET} statement although 
it requires a first argument that specifies a logical file identifier.  The
...
\note{
	...
	command with no space.  So instead of\\
	{\ttfamily GET\# 1,A\$} this example shows {\ttfamily GET\#1,A\$}.
